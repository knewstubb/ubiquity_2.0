...
interface NotificationsStepProps {
  value: ExporterNotificationConfig;
  onUpdate: (config: ExporterNotificationConfig) => void;
...
export function NotificationsStep({ value, onUpdate, onValidChange, teamEmails, showNoFileAlert }: NotificationsStepProps) {
  ...
  /* Success */
  const [successEnabled, setSuccessEnabled] = useState(value.successEnabled);
  const [successEmails, setSuccessEmails] = useState<string[]>(value.successEmails);

  /* No File */
  const [noFileEnabled, setNoFileEnabled] = useState(value.noFileAlertEnabled);
  const [noFileEmails, setNoFileEmails] = useState<string[]>(value.noFileAlertEmails);

  /* No File schedule state */
  const [noFileFrequency, setNoFileFrequency] = useState<Frequency>(value.noFileSchedule?.frequency ?? 'hourly');
  const [noFileStarting, setNoFileStarting] = useState(value.noFileSchedule?.starting ?? 'Friday, 09 May, 2025');
  const [noFileEvery, setNoFileEvery] = useState(value.noFileSchedule?.every ?? '1');
  const [noFileAt, setNoFileAt] = useState(value.noFileSchedule?.at ?? '2:30 pm');
  const [noFileDays, setNoFileDays] = useState<boolean[]>(
    value.noFileSchedule?.weeklyDays ?? [false, true, true, false, true, false, false]
  );
  const [monthlyPattern, setMonthlyPattern] = useState<'day' | 'date'>(value.noFileSchedule?.monthlyPattern ?? 'day');
  const [monthlyOrdinal, setMonthlyOrdinal] = useState(value.noFileSchedule?.monthlyOrdinal ?? '2nd');
  const [monthlyDayOfWeek, setMonthlyDayOfWeek] = useState(value.noFileSchedule?.monthlyDayOfWeek ?? 'Wednesday');
  const [monthlyDates, setMonthlyDates] = useState<string[]>(value.noFileSchedule?.monthlyDates ?? ['1st', '8th', '15th']);
  ...
  useEffect(() => {
    const config: ExporterNotificationConfig = {
      failureEmails,
      successEnabled,
      successEmails,
      noFileAlertEnabled: noFileEnabled,
      noFileAlertEmails: noFileEmails,
      noFileSchedule: noFileEnabled
        ? {
            frequency: noFileFrequency,
            starting: noFileStarting,
            every: noFileEvery,
            at: noFileAt,
            weeklyDays: noFileDays,
            monthlyPattern,
            monthlyOrdinal,
            monthlyDayOfWeek,
            monthlyDates,
