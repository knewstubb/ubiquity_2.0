...
export const componentRegistry: ComponentEntry[] = [
  ...
  },
  {
    ...
    slug: 'filter-builder',
    category: 'inputs',
    description: 'Multi-row filter builder for constructing field conditions combined with AND or OR logic. Fields declare their data type (text, number, date, boolean, enum) which determines available operators and value input.',
    searchTerms: ['filter', 'query builder', 'field filter', 'conditions', 'where clause', 'rule builder', 'AND filter', 'OR filter', 'segment builder'],
    usesComponents: ['Select', 'Input', 'Button', 'Popover', 'Calendar'],
    designGuidance: [
      { heading: 'When to use', content: [
        'Building filter conditions for data exports (contacts, messages, transactions)',
        'Segment builder audience conditions by contact attributes',
        'Any pattern needing composable field + operator + value filter rows',
        'Query builders where multiple conditions are combined with AND or OR logic',
      ]},
      { heading: 'When NOT to use', content: [
        'Simple single-field filters — use a Select or Input directly',
        'Complex nested conditions mixing AND + OR at different levels — requires a grouped query builder',
        'Predefined filter presets (e.g. "Last 7 days") — use RadioCard options instead',
      ]},
      { heading: 'Logic', content: [
        'AND/OR toggle: clickable toggle appears between row 1 and 2 when onLogicChange is provided',
        'Subsequent rows display the chosen logic as a static label (all rows share the same logic)',
        'Default: AND logic when no onLogicChange handler is provided (read-only label)',
      ]},
      { heading: 'Data types', content: [
        'text: equals, not equals, contains, starts with, ends with, is empty/not empty',
        'number: equals, not equals, greater/less than, greater/less or equal, is empty/not empty',
        'date: on, before, after, between (calendar picker), in last N days (numeric input), is empty/not empty',
        'boolean: is true, is false (no value input needed)',
        'enum: is, is not, is empty/not empty (value rendered as a Select from enumOptions)',
      ]},
      { heading: 'States', content: [
        'Field selected: operator selector appears (progressive disclosure)',
        'Operator selected: value input appears (unless no-value operator)',
        'Logic toggle active: selected logic has primary/10 bg + primary text; inactive is muted until hovered',
        'Date value (on/before/after): inline Popover with single Calendar picker — displays formatted date in NZ locale',
        'Date value (between): dual-calendar range picker (side-by-side months) — displays "start — end" range',
        'Field changed: operator and value reset automatically',
        'Partially filled: inline validation error appears below the row',
        'No-value operators (is_true, is_false, is_empty, is_not_empty): value input hidden',
        'Max rows reached: "Add filter" button disappears',
        'Single row: remove button resets to empty row (never zero rows)',
      ]},
    ],
    propControls: [
      ...
      { name: 'max-rows', label: 'Max Rows', controlType: 'counter', defaultValue: 5, min: 1, max: 10 },
    ],
