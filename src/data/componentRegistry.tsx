...
export const componentRegistry: ComponentEntry[] = [
  ...
  // Inputs
  {
    ...
    designGuidance: [
      ...
      { heading: 'States', content: [
        ...
        'destructiveGhost (red text, no border): Lowest-weight destructive — icon-only remove/delete buttons',
        ...
        'ghost (teal text, no bg until hover): Inline actions in primary colour — teal text with subtle teal hover',
        'secondaryGhost (neutral text, no bg until hover): Minimal/muted — icon-only buttons, inline actions where colour prominence is not needed',
      ...
      { heading: 'Sizes', content: [
        'xs (h-6 / 24px): Extra compact — inline badges, filter pills, tight toolbar buttons',
        'sm (h-8 / 32px): Compact contexts — table rows, filter bars',
        'default (h-9 / 36px): Standard usage — forms, modals, page actions',
        'lg (h-10 / 40px): Hero CTAs, modal primary actions',
        'icon (h-9 / 36px): Icon-only square buttons',
      ]},
  ...
  },
  {
    ...
    designGuidance: [
      ...
      { heading: 'When NOT to use', content: [
        ...
        '"Cancel" actions that need a visible label — use Button variant="secondaryGhost" instead',
      ]},
      { heading: 'States', content: [
  ...
  },
  {
    ...
    designGuidance: [
      ...
      { heading: 'States', content: [
        ...
        'Selected (single): filled primary background on chosen date',
        'Selected (range): filled primary endpoints with accent background span between',
        'Today: teal text + semibold weight (no background fill) — distinguishes today without competing with selection states that use filled backgrounds',
        'Disabled dates: muted text, 50% opacity, no pointer events',
        'Outside days: tertiary-foreground colour at 50% opacity — recedes strongly to emphasise in-month dates without disappearing',
      ]},
    ],
    propControls: [
  ...
  },
  {
    ...
    designGuidance: [
      { heading: 'When to use', content: [
        'Numeric values with a small, bounded range (quantity, days, count)',
        'When fine-grained control via click-to-increment is preferred over typing',
        'Settings panels where precise integer adjustment is needed',
      ]},
      { heading: 'When NOT to use', content: [
        'Large numeric ranges — use a standard number Input instead',
        'Unbounded values like prices or IDs — use Input type number',
        'Non-numeric input — use Input or Select',
      ]},
      { heading: 'States', content: [
        'Default: editable text with stepper arrows on the right',
        'Focused: focus-within on wrapper provides border-ring + shadow-ring',
        'Disabled: reduced opacity, no interaction on input or steppers',
        'At min/max: respective arrow becomes disabled (opacity 30%)',
      ]},
    ],
    propControls: [
      { name: 'label', label: 'Label', controlType: 'text', defaultValue: '' },
      { name: 'min', label: 'Min', controlType: 'number', defaultValue: 0 },
      { name: 'max', label: 'Max', controlType: 'number', defaultValue: 100 },
      { name: 'step', label: 'Step', controlType: 'number', defaultValue: 1 },
      { name: 'disabled', label: 'Disabled', controlType: 'toggle', defaultValue: false },
    ],
  },
  {
  ...
  },
  {
    ...
    propControls: [
      ...
      { name: 'cards', label: 'Cards', controlType: 'counter', defaultValue: 3, min: 2, max: 6 },
      { name: 'columns', label: 'Columns', controlType: 'counter', defaultValue: 3, min: 1, max: 4 },
  ...
  },
  {
    ...
    designGuidance: [
      ...
      },
      {
        heading: 'Dismissal',
        content: [
          'Clicking the overlay does NOT close the dialog — users must explicitly dismiss via a close/cancel button.',
          'Escape key still closes the dialog (Radix default).',
          'This prevents accidental data loss in form dialogs.',
        ],
      },
    ],
  },
  ...
  },
  {
    ...
    designGuidance: [
      ...
      { heading: 'States', content: [
        'Active page: teal border + teal text + font-semibold + rounded-md, hover shows bg-accent',
        'Active page (sm): same styling but h-7 min-w-7 text-xs',
        'Inactive pages: muted-foreground text, hover transitions to primary (teal) with transparent background',
        'Ellipsis: DotsThree icon centered in 36×36px slot, non-interactive',
        'Previous/Next (default): h-9 links with 16px caret icons and label text',
        'Previous/Next (sm): h-7 compact links with 12px caret icons — for modals and dense tables',
      ]},
      { heading: 'Accessibility', content: [
        'nav element with role="navigation" and aria-label="pagination"',
        'Active page marked with aria-current="page"',
        'Previous/Next have descriptive aria-label for screen readers',
  ...
  },
  {
    ...
    designGuidance: [
      { heading: 'When to use', content: [
        ...
        'Parent-child data where expanding a row reveals detail rows or sub-content',
        'Any table pattern that needs a caret toggle to show/hide nested content',
      ]},
      { heading: 'When NOT to use', content: [
        'Standard flat data tables — use DataTable instead',
        'Sortable/selectable data grids — use DataTable',
        'Simple tree navigation — use a tree component or nested lists',
      ]},
      { heading: 'States', content: [
        'Expandable row: hover:bg-secondary, cursor-pointer, caret right',
        'Expanded row: caret down, disclosure panel animates open (200ms ease) with bg-muted/40',
        'Non-expandable row: opacity-50, no caret, aria-disabled',
        'Accordion: only one row expanded at a time — opening one closes the other',
        'ARIA: role="table", aria-expanded on buttons, aria-controls linking to panel id',
      ]},
      { heading: 'Compound pattern', content: [
        'ExpandableTable — wrapper with column definitions, provides accordion context',
        'ExpandableRow — individual row with caret, cells, and expandable children slot',
        'DISCLOSURE_INDENT — exported constant (88px) for child content left margin alignment',
      ]},
    ],
    propControls: [
      { name: 'row-count', label: 'Rows', controlType: 'counter', defaultValue: 3, min: 1, max: 5 },
      { name: 'show-non-expandable', label: 'Show Non-Expandable', controlType: 'toggle', defaultValue: true },
      { name: 'default-expanded', label: 'Default Expanded', controlType: 'toggle', defaultValue: false },
    ],
    component: lazy(() => import('../pages/component-demos/ExpandableTableDemo')),
  },
  {
    name: 'FilterBuilder',
    slug: 'filter-builder',
    ...
    designGuidance: [
      { heading: 'When to use', content: [
        ...
        'Segment builder audience conditions with nested AND/OR logic',
        'Journey builder: complex nested conditions across multiple levels',
        'Any pattern needing composable field + operator + value filter groups',
        'Hierarchical field sources — pass sourceCategories for searchable combobox with full path display',
      ]},
      { heading: 'When NOT to use', content: [
        'Simple single-field filters — use a Select or Input directly',
        'Predefined filter presets (e.g. "Last 7 days") — use RadioCard options instead',
        'Non-combinable tag-style filtering — use ChipInput',
      ]},
      { heading: 'Behaviour', content: [
        'Progressive drill-down with source categories, sub-sources, and human-readable condition cards. Conditions are added/edited via an inline card (InlineConditionCard) that appears in-place within the filter list — no overlay modal required. Uses DrilldownNavigator for slide-animated hierarchical source/sub-source/field selection — static title shows the current phase prompt, each breadcrumb segment is individually clickable to jump directly to that level. Header row (field label + source path) is hidden during drilldown phases — the DrilldownNavigator provides its own navigation context.',
        'Transactional sub-sources: When a sub-source has sourceType "transactional", the inline card skips field selection and presents relationship operators (has transactions, has matching transactions, etc.). Matching operators reveal a sub-filter WHERE clause with field/operator/value rows and AND chaining, plus an aggregate section ("And those transactions have") for measure conditions (number_of_rows, total_value, highest_value, lowest_value, average) — models the "does this contact have records where...?" pattern from the platform audit.',
      ]},
      { heading: 'Progressive disclosure', content: [
        ...
        'Phase containers animate in via drillForward (2000ms ease, translateX 24px→0 + opacity) — conveys forward navigation from drill-down into operator-value configuration. Custom @keyframes in globals.css applied via inline style for reliable play-once behaviour.',
        'Sub-sections (WHERE clause, aggregate) use slideUp (200ms ease-out) for vertical reveal within the phase.',
        'Inline reveals (date mode, value) use slideInRight (250ms ease-out) for left-to-right progressive appearance.',
        'Sub-filter WHERE rows use the same progressive reveal: field starts at flex-[3] then shrinks as operator/value appear. Operator and value are conditionally rendered (not disabled) with slideInRight entrance.',
        'Aggregate Row 2 (field/operator/value) mirrors the sub-filter pattern: field at flex-[3] → flex-[1.5] → flex-1 as siblings appear; operator and value slide in via slideInRight. Trailing X resets the row.',
        'Additional aggregate fields separated by AND pill dividers (teal pill with horizontal lines) — same progressive reveal per row.',
      ]},
      { heading: 'States', content: [
        'Empty (default): Flex-fill top-biased layout (flex spacer at 20% height, justify-start) with ArrowsClockwise icon + "No filter applied" heading + explanatory text (max-w-sm) + auto-width dashed-border "Add a filter condition" CTA. Copy explains delta export model (all new/modified records included by default). Positioned in upper third rather than dead-centre for a more natural feel in tall panels. No logic group chrome until first condition exists.',
        'Empty (custom): Pass `emptyState` prop (ReactNode) to replace default icon/heading/text with context-specific content. The dashed "Add a filter condition" CTA always renders below custom content.',
        'Populated: Full-width row cards with drag handle + summary text + hover actions (clone, remove, disable). Drag-and-drop reorder uses position-aware targeting — cursor in top half of target card inserts above, bottom half inserts below, with a primary-coloured border indicator showing exact insertion point.',
        'Nested: Sub-groups with opposite logic, single level only (no nesting inside groups). Group actions (clone, remove) revealed on group hover via CSS (hidden when a child condition card is hovered to avoid conflicting highlights). Nested "Add condition" uses outline variant and "Add condition to group" label to differentiate from root-level add. Bottom action buttons in nested groups are collapsed (max-h-0, opacity-0) until the group card is hovered — keeps nested groups compact and reduces visual noise in complex filter trees.',
        ...
        'Disabled condition: Per-element opacity-40 on drag handle, icon, and content (not entire card) — card border stays fully visible. Eye/EyeSlash toggle always visible; still editable and removable. Allows temporary exclusion without deletion.',
        'Action button hover colours: Clone uses teal (primary) for constructive action, Remove uses red (destructive), Disable uses amber for caution — semantic colour signals intent without labels.',
        ...
        'Add-group inline: Dashed-border frame with "Group · {opposite logic}" header (sentence case) wrapping an InlineConditionCard — previews nested group structure before condition is configured.',
        'At max depth: "+Group" button disabled with tooltip.',
        'At condition limit (maxConditions): "+ Add condition" buttons disabled across all groups — recursive tree walk counts all row-type conditions. Clone button on condition cards also disabled (opacity-40, "Condition limit reached" tooltip) to prevent exceeding the cap via duplication.',
        'At group limit (maxGroups): "+ Add group" buttons disabled across all groups — recursive tree walk counts all nested group-type conditions. Clone button on group cards also disabled (opacity-40, cursor-not-allowed) to prevent exceeding the cap via duplication.',
        ...
        'Relationship condition: For transactional sub-sources, field phase is skipped — operator select shows relationship operators (has/has no/has matching transactions). Sub-filter WHERE clause renders as a bordered sub-card below the operator for matching operators — initializes with one empty row so the user can start configuring immediately without an explicit "add" click. Aggregate section ("And those transactions have") renders below the WHERE clause — aggregate Row 2 uses progressive flex reveal (field starts at flex-[3], shrinks as operator/value appear with slideInRight animation). Each aggregate row includes a trailing X button to reset field/operator/value without removing the entire aggregate. Additional aggregate field rows are separated by AND pill dividers (teal text, horizontal lines) reinforcing AND logic. Summary bar shows comprehensive natural-language preview: sub-source + relationship operator + WHERE clause conditions (joined by logic) + aggregate measure ("with {type} of {field} {operator} {value}").',
      ]},
      { heading: 'Container styling', content: [
        'Borderless container (padding only) — adapts to parent context (card, panel, page section).',
        'Empty state (no sourceCategories): bordered container with warning message for standalone visibility.',
      ]},
    ],
    propControls: [
      { name: 'allow-nesting', label: 'Allow Nesting', controlType: 'toggle', defaultValue: true },
      { name: 'max-depth', label: 'Max Depth', controlType: 'counter', defaultValue: 3, min: 1, max: 5, visibleWhen: { controlName: 'allow-nesting', values: [true] } },
    ],
    component: lazy(() => import('../pages/component-demos/FilterBuilderDemo')),
  },
  {
    name: 'Toast',
    slug: 'toast',
  ...
  },
  {
    ...
    propControls: [
      ...
      { name: 'text', label: 'Text', controlType: 'text', defaultValue: 'Contacts already in the segment will not be re-added.' },
    ],
    designGuidance: [
      { heading: 'When to use', content: [
        'Below card selectors to explain the behaviour of a selection',
        'Below frequency/schedule controls to note system behaviour',
        'Anywhere a non-blocking "FYI" annotation is needed without requiring action',
      ]},
      { heading: 'When NOT to use', content: [
        'Warnings or errors — use Alert component instead',
        'Actionable messages (dismiss, retry) — use Toast or Alert with actions',
        'Long explanatory text — use a help panel or documentation link',
      ]},
      { heading: 'Variants', content: [
  ...
  },
  {
    ...
    designGuidance: [
      { heading: 'When to use', content: [
        ...
        'Single date selection when you need the Popover trigger pattern (mode="single")',
      ]},
      { heading: 'When NOT to use', content: [
        'Inline single date selection without a trigger — use Calendar component directly',
        'Relative period selection without calendar (e.g. "Last 7 days" toggle) — use SegmentedControl or Select',
      ]},
      { heading: 'States', content: [
        'Closed: styled trigger button showing formatted date range with calendar icon',
        ...
        'Single mode: closes on date selection; Range mode: stays open for refinement, closes on preset click or outside click',
      ]},
      { heading: 'Spacing', content: [
        'Calendar owns its own internal padding — no extra wrapper padding applied by DateRangePicker',
        'Presets panel: py-3 with px-4 per item, border-l separator',
      ]},
    ],
    propControls: [
      { name: 'mode', label: 'Mode', controlType: 'select', defaultValue: 'range-presets', options: [
        { label: 'Single Day', value: 'single' },
        { label: 'Date Range', value: 'range' },
        { label: 'Preset Options', value: 'range-presets' },
      ]},
      { name: 'side-by-side', label: 'Side by Side', controlType: 'toggle', defaultValue: false, visibleWhen: { controlName: 'mode', values: ['range', 'range-presets'] } },
    ],
  },
  {
    name: 'HelpPopover',
  ...
  },
  {
    ...
    propControls: [
      ...
      { name: 'description', label: 'Description', controlType: 'text', defaultValue: '**10:30 am** on the **2nd Tuesday** of **every month**' },
      { name: 'audienceCount', label: 'Audience Count', controlType: 'number', defaultValue: 533000 },
      { name: 'isIncomplete', label: 'Incomplete', controlType: 'toggle', defaultValue: false },
      { name: 'hasError', label: 'Has Error', controlType: 'toggle', defaultValue: false },
      { name: 'selected', label: 'Selected', controlType: 'toggle', defaultValue: false },
    ],
    designGuidance: [
      { heading: 'Anatomy', content: [
        'Compact card: fixed width 240px, rounded-lg (8px), shadow-sm',
        'Header row: Icon (20px, regular weight) + Title (12px semibold, secondary color) + Audience badge',
        'Separator: 1px border-border between header and body',
        'Body: Description text (14px, centered) supporting bold emphasis via **text**',
        'Connection handles: 10px circles, muted-foreground color',
      ]},
      { heading: 'Node types and colours', content: [
        'Schedule/Email/Action: text-primary (teal)',
        'Condition: text-sky-500 (blue)',
        'Trigger: text-amber-500 (orange)',
        'Wait: text-blue-500 (blue)',
        'SMS: text-violet-500 (purple)',
        'Exit: text-red-500 (red)',
      ]},
      { heading: 'States', content: [
        'Default: border-border, shadow-sm',
        'Selected: ring-2 ring-primary ring-offset-2',
        'Incomplete: border-dashed border-muted-foreground (config not finished)',
        'Error: border-red-500 with small error badge in top-right corner',
      ]},
      { heading: 'Audience badge', content: [
        'Shows count of contacts at this step in the journey',
        'Formatted: 1000→1k, 1000000→1M',
        'Uses Users icon (14px) from Phosphor',
      ]},
    ],
  },
]
